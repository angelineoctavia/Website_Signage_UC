    ...
    <div class="max-w-[1600px] w-full mx-auto p-6 lg:p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-8 space-y-6">
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="bg-uc-green text-white rounded-2xl p-5 shadow-sm flex flex-col justify-between h-32">
                        <span class="text-xs font-medium opacity-90">Active Playlist (Hari Ini)</span>
                        <div class="text-2xl font-bold text-center my-auto">
                            @if (isset($activePlaylistToday) && $activePlaylistToday)
                                Playlist {{ $activePlaylistToday->playlist_id }}
                            @else
                                -
                            @endif
                        </div>
                                ...
                                <div class="border border-gray-200 rounded-xl overflow-hidden">
                                    ...
                                    @foreach ($calendarWeeks as $week)
                                        <div class="border-t border-gray-200">
                                            ...
                                            @if ($week['row_count'] > 0)
                                                <div class="grid grid-cols-7 gap-1 px-1 pb-1.5"
                                                    style="grid-template-rows: repeat({{ $week['row_count'] }}, 26px);">
                                                    @foreach ($week['bars'] as $bar)
                                                        <div class="playlist-bar {{ $bar['bg'] }} {{ $bar['border'] }} {{ $bar['text'] }} border text-[10px] font-semibold px-2 flex items-center gap-1 rounded-lg truncate shadow-sm cursor-pointer {{ $bar['is_live'] ? 'ring-2 ring-emerald-500' : '' }}"
            ...
            <div class="lg:col-span-4 lg:self-center flex flex-col items-center justify-center w-full">
                ...
                <p class="mt-4 text-center text-[11px] text-gray-400 max-w-[300px]">
                    TV signage otomatis menayangkan playlist sesuai jadwal tanggal — tidak perlu klik apa pun.
                </p>
            </div>
        </div>
            ...
            <div class="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl relative">
                ...
                <div class="flex items-center justify-between">
                    <button id="modalShowNowBtn" type="button"
                        ...
                        <i class="fa-solid fa-eye text-[10px]"></i><span>Preview Playlist</span>
                    </button>
                                ...
                                <tbody id="contentTableBody" class="divide-y divide-gray-200 text-gray-700">
                                    ...
                                    @forelse($sortedContents as $index => $content)
                                        ...
                                        <tr class="content-row {{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-orange-50/50 transition-colors"
                                            ...
                                            data-category="{{ strtolower($content->content_category) }}"
                                            data-uploader="{{ strtolower($content->users_name) }}">
                                            ...
                                            <td class="p-3 text-right">
                                                <a href="{{ $downloadUrl }}"
                                                    class="inline-flex items-center space-x-1 bg-uc-blue hover:bg-blue-600 text-white px-2.5 py-1.5 rounded-lg text-[10px] font-semibold transition-colors">
